...
Class.boundTestBullet = {
   ...
   //  MOTION_EFFECTS: false,
       ...
       },
    {
        event: "death",
        handler: ({ body, killers, killTools }) => {
        let indexToRemove = body.master.children.indexOf(body);
          if (indexToRemove !== -1) {
        body.master.children.splice(indexToRemove, 1);
    }
        }
    }, 
],
        ...
        const bounda = { size:1, offset:2, direction:0, angle:0, arc:36000 };
    ...
    GUNS: [
        {
            POSITION: [18, 8, 1, 0, 0, 0, 0],
            PROPERTIES: {
                SHOOT_SETTINGS: combineStats([{recoil:0, reload:3, range:0, maxSpeed:Infinity, health:0, damage:10, pen:0, speed:0,}]),
                TYPE: "bullet",
                ALT_FIRE:true,
              //  MAX_CHILDREN:20,
            }
       ...
       ON: [
        ...
        event: "altFire",
        handler: ({ body, gun }) => {
            body.collisionDetected = false;
          const size = (body.size * 8/20);
        let target = {
            x: body.x + body.control.target.x - (Math.cos(body.facing) * (body.size * bounda.offset)),
            y: body.y + body.control.target.y - (Math.sin(body.facing) * (body.size * bounda.offset)),
        },  
        amount = util.getDistance(target, body) / ((size) * 2);
      let bodyChildrenAmount = body.children.length -1;
   if (amount == bodyChildrenAmount) {amount = 0} else
      if (amount < bodyChildrenAmount && amount >= 0) {
    for (let i = bodyChildrenAmount; i >= 0; i--) {
        if (i >= amount + 1 && body.children[i] != body) {
            body.children[i].kill()
        }
    }
    amount = 0
      } else if (amount > bodyChildrenAmount) {
      amount = amount - (bodyChildrenAmount)
    }
     
        for (let i = 0; i < amount; i++) {
             let plannedX =  body.x +  (size * bounda.offset * Math.cos(bounda.direction + bounda.angle + body.facing)) * (i-1 + body.children.length),
                 plannedY =  body.y +  (size * bounda.offset * Math.sin(bounda.direction + bounda.angle + body.facing)) * (i-1 + body.children.length);
            for (let instance of entities) {
            if (Math.sqrt(((instance.x - plannedX) ** 2) + ((instance.y - plannedY) ** 2)) <= size + instance.size && instance.master.id != body.id) {
                body.collisionDetected = true;
                break;
            }
        }
      ...
      if (body.collisionDetected) {
        ...
        let e = new Entity({ x: body.x, y: body.y });
        body.children.push(e);
        e.define(Class.boundTestBullet);
        e.define({
          BODY: {
      ...
      
       {
        event: "tick",
        handler: ({ body }) => {  
if (!body.children.includes(body)) {
  body.children.unshift(body);
}          const bounda = { size:1, offset:2, direction:0, angle:0, arc:36000 };
          let snake = body.children.filter(r => r.isGhost == false);
          for (let [i, player] of  body.children.entries()) {
           if (i === 0) continue;
            
            if (player.isGhost || player.health.amount <0 && player.range > 0) {
        snake.forEach((snakeElement, index) => {
            if (index >= i) {
                snakeElement.kill();
            }
        });
            ...
            for (let instance of entities) {
  if (Math.sqrt(((instance.x - player.x) ** 2) + ((instance.y - player.y) ** 2)) <= player.size + instance.size && instance.master.id != body.id) {
             if (player.range > 0) {
       for (let j = i + 1; j < snake.length; j++) {
          ...
          let refa = snake[i-1];
           player.x =  refa.x +  refa.size * bounda.offset * Math.cos(bounda.direction + bounda.angle + body.facing);//refa.facing);
           player.y =  refa.y +  refa.size * bounda.offset * Math.sin(bounda.direction + bounda.angle + body.facing);//refa.facing);
          // snake[i-1].velocity.x += bounda.size * player.accel.x * snake[i-1].recoilMultiplier;
            //  snake[i-1].velocity.y += bounda.size * player.accel.y * snake[i-1].recoilMultiplier;
          //   player.velocity =  snake[i-1].velocity;
          // player.accel.null();
          // player.blend =  snake[i-1].blend;
...
Class.developer.UPGRADES_TIER_0 = ["basic", "tanks", "AIT", "utilities", "addons"]
    Class.tanks.UPGRADES_TIER_0 = ["developer", "overpowered", "testing", "unavailable", "features"]
        ...
        Class.testing.UPGRADES_TIER_0 = ["tanks", "whirlwind", "vanquisher", "mummifier", "tracker3", "boundTest"]
